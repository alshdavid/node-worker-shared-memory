...
pub fn js_string_new(sync_state: SyncState) -> impl Fn(FunctionContext) -> JsResult<JsNumber> {
...
pub fn js_string_get(sync_state: SyncState) -> impl Fn(FunctionContext) -> JsResult<JsString> {
...
pub fn js_string_set(sync_state: SyncState) -> impl Fn(FunctionContext) -> JsResult<JsBoolean> {
...
pub fn js_string_delete(sync_state: SyncState) -> impl Fn(FunctionContext) -> JsResult<JsBoolean> {
    return move |mut cx: FunctionContext| -> JsResult<JsBoolean> {
        let mut state = sync_state.lock().unwrap();
